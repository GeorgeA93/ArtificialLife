...
public class TournamentParentSelection extends ParentSelector {
    ...
    public LifeForm selectParent(ArrayList<LifeForm> population) {
        LifeForm[] tournament = new LifeForm[4];
        double[] tournamentFitness = new double[4];
        for(int i = 0; i < 4; i++){
    ...
    public LifeForm selectParentT(ArrayList<LifeForm> population, int tournamentSize){
    ...
    public void selectParents(ArrayList<LifeForm> genotypes){
        ...
        for(int i = 0; i < parentsA.length; i ++) {
            ...
            LifeForm parent = selectParentT(population, tournamentSize);
            population.remove(parent);
            parentsA[i] = parent;
            Collections.shuffle(population);
            LifeForm parent2 = selectParentT(population, tournamentSize);
